...
_re_hospital_big = re.compile(r"(종합병원|대학병원|대형병원)", re.IGNORECASE)
...
def load_traffic_sensors_exact(csv_path: str):
    ...
    return out[["id","name","lat","lon","road_address","jibun_address","category","source"]]
...
def add_traffic_sensors_layer(m, df_sensors):
    ...
    for _, r in df_sensors.iterrows():
        ...
        popup = build_popup_html(
            title=name,
            rows=[("구분","스마트 교통량 수집기"),("도로명",road_addr),("지번",jibun_addr)]
        )
        folium.Marker([float(r["lat"]), float(r["lon"])], tooltip=tooltip, popup=popup,
                      icon=folium.Icon(color="blue", icon="wifi", prefix="fa")).add_to(cluster)
    m.add_child(fg)
    return m
